...
export function seedAll() {
  ...
  createLocalStore<typeof mainDetail>("hackathon_details", "slug")
    .seed([mainDetail, ...(extraDetails ?? [])])

  // 리더보드 — extraLeaderboards를 꺼내 평탄화
  const { extraLeaderboards, ...mainLeaderboard } = leaderboardData
  createLocalStore<typeof mainLeaderboard>("leaderboards", "hackathonSlug")
    .seed([mainLeaderboard, ...(extraLeaderboards ?? [])])
